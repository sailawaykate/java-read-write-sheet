...
public class rwsheet {
    ...
    private static Row evaluateRowAndBuildUpdates(Row sourceRow) {
        ...
        // Find cells we want to examine
        Cell statusCell = getCellByColumnName(sourceRow, "Calculated % Allocation");
        Cell remainingCell = getCellByColumnName(sourceRow, "Allocation %");

        if (statusCell.getDisplayValue() != remainingCell.getDisplayValue()) {
            //Cell remainingCell = getCellByColumnName(sourceRow, "Allocation %");
            // if (! "0".equals(remainingCell.getDisplayValue()))                  // Skip if "Remaining" is already zero
            //{
            System.out.println("\nNeed to update row #" + sourceRow.getRowNumber());
            System.out.println("Copying: " + statusCell.getDisplayValue().replaceAll("[^0-9.]", "")); //Removing the %
            System.out.println("Overriding: " + remainingCell.getDisplayValue());
            ...
            List<Cell> cellsToUpdate = new Cell.UpdateRowCellsBuilder()
                    .addCell(columnMap.get("Allocation %"), Double.parseDouble(statusCell.getDisplayValue().substring(0,2)) /100)        // Set value to match Calculated % Allocation
                    .build();
